...
import { ArrowRight, Shield, Calendar, Users, Lock } from "lucide-react";
...
export default async function ElectionDetailPage({
  ...
  const { election, candidates, permissions } =
    await getElectionById(electionId);

  // If user cannot view (not admin and not in allowed list), show access denied
  if (!permissions.canView) {
    return (
      <div className="flex-grow w-full max-w-[1200px] mx-auto px-6 py-20 mt-10 text-center">
        <Lock className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
        <h1 className="text-3xl font-bold text-white mb-2">Access Denied</h1>
        <p className="text-muted-foreground">
          You are not authorized to view this election.
        </p>
      </div>
    );
  }
  ...
  const isTallied =
    election.status === "TALLIED" || election.status === "CLOSED";
    ...
    <div className="flex-grow w-full max-w-[1200px] mx-auto px-6 py-20 mt-10">
      ...
      <div className="mb-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-6">
          <div>
            <h1 className="text-5xl font-bold font-serif text-white mb-3">
              {election.title}
            </h1>
            <p className="text-xl text-muted-foreground max-w-2xl font-sans">
              {election.description ||
                "Secure, mathematically verifiable voting event."}
            </p>
          </div>
          <div className="flex flex-col items-start md:items-end gap-2">
            <div
              className={`px-4 py-1.5 rounded-full border text-xs font-bold tracking-widest uppercase flex items-center gap-2 shadow-[0_0_15px_rgba(var(--primary),0.1)] ${
                isOpen
        ...
        <div className="flex flex-wrap gap-8 py-6 border-y border-[#262626]">
          <div className="flex items-center gap-3 text-muted-foreground font-mono text-sm uppercase tracking-tighter">
            ...
            <span>
              Threshold: {election.threshold}/{election.totalShares} Shares
            </span>
          </div>
          <div className="flex items-center gap-3 text-muted-foreground font-mono text-sm uppercase tracking-tighter">
          ...
          {isTallied && (
            <div className="sticky top-32">
              ...
              <div className="bg-[#121212] border border-[#262626] p-8 rounded-xl">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed mb-6">
                  The cryptographic tallying process has been completed and
                  verified against all zero-knowledge proofs.
                </p>

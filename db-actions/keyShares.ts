...
export async function getSharesByElection(electionId: number): Promise<KeyShare[]> {
  try {
    return await db.select().from(keyShares).where(eq(keyShares.electionId, electionId));
  } catch (err) {
    console.error(`[db-actions] Error getting shares for election ${electionId}:`, err);
    throw err;
  }
}
...
export async function getSharesByElectionAndAdmin(electionId: number, adminId: number): Promise<KeyShare[]> {
  try {
    ...
    console.error(`[db-actions] Error getting shares for election ${electionId} and admin ${adminId}:`, err);
    throw err;
  }
}
...
export async function getSubmittedSharesByElection(electionId: number): Promise<KeyShare[]> {
  try {
    ...
    console.error(`[db-actions] Error getting submitted shares for election ${electionId}:`, err);
    throw err;
  }
}
...
export async function getShareCount(electionId: number): Promise<number> {
  try {
    ...
    console.error(`[db-actions] Error getting share count for election ${electionId}:`, err);
    throw err;
  }
}
